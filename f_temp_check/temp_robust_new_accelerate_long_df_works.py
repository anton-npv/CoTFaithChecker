...
# accelerate launch f_temp_check/temp_robust_new_accelerate_long_df.py
...
def generate_n_completions_batched(
    ...
    logging.info(
        f"Process {accelerator.process_index}: Generating {num_generations} completions for each of {num_local_prompts} questions in a single pass …"
    )

    # Iterate over batches of *prompts* once.  We rely on `num_return_sequences`
    # to obtain `num_generations` completions per prompt, so there is **no**
    # outer loop over runs any more.
    iterable = range(0, num_local_prompts, batch_size)
    if accelerator.is_main_process:
        iterable = tqdm(iterable, desc="Batches (Main Process)")
    else:
        iterable = tqdm(
            iterable,
    ...
    for i in iterable:
        ...
        # Tokenise (already on the right device thanks to accelerator)
        encodings = tokenizer(
            batch_prompts,
            padding=True,
        ...
        try:
            with torch.no_grad():
                ...
                # Ask the model for `num_return_sequences` completions **per row**.
            ...
            # `outputs` now has (batch_size * num_generations, …) rows.
            generated_ids = outputs[:, input_length:]
            generated_parts = tokenizer.batch_decode(
                generated_ids, skip_special_tokens=True
            )

            # Map rows back to (question_id, run_index)
            for j, qid in enumerate(batch_qids):
                prompt_text = batch_prompts[j]

                for k in range(num_generations):
                    row_idx = j * num_generations + k
                    gen_part = generated_parts[row_idx]
                    final_completion = (
                        prompt_text + gen_part.strip() + tokenizer.eos_token
                    )
            ...
            logging.error(
                f"Process {accelerator.process_index}: Error during generation for batch starting with QID {batch_qids[0]}: {e}"
            )
            for qid in batch_qids:
                # Ensure the correct number of placeholders so downstream code sees the right length
                local_results_store[qid].extend(
                    [f"GENERATION_ERROR: {e}"] * num_generations
                )
...
def run_generation_phase(args):
    ...
    if accelerator.is_main_process:
        try:
            ...
            logging.error(f"Main process failed to load necessary input data files: {e}")
            # Signal other processes if main process fails critically here
            accelerator.set_trigger()
            return
    else:
        # Non-main processes don't load the full data initially
        pass
    ...
    # --- 4. Prepare Prompts & Target Info (Main Process Prepares, then Broadcasts/Splits) ---
    # Let main process prepare the full list of prompts and target info
    all_prompts_for_generation = []
    all_target_info = {}
    if accelerator.is_main_process:
        logging.info("Main process preparing prompts...")
        ...
        for item in target_questions_data:
            ...
            all_prompts_for_generation.append({'question_id': qid, 'prompt_text': prompt_text})
            all_target_info[qid] = {'original_verbalizes_hint': original_verbalizes, 'hint_option': hint_option}
        ...
        if not all_prompts_for_generation:
            logging.error("Main process: No valid prompts prepared for generation. Setting trigger.")
            accelerator.set_trigger()
        else:
            logging.info(f"Main process prepared {len(all_prompts_for_generation)} prompts.")
    ...
    # Distribute the prepared data. Broadcast target_info and split prompts.
    # Need to gather target_info on all processes for the final structuring step
    all_target_info_list = [all_target_info] # Wrap in list for gather_object compatibility
    gathered_target_info_list = gather_object(all_target_info_list)

    # ------------------------------------------------------------
    # Broadcast the full prompt list so **every** rank participates
    # in the same collective. Only rank-0 sends the data, the rest
    # receive it (initially holding `None`).
    # ------------------------------------------------------------

    # 1) Target-info dict: already gathered above and stored in list index 0
    final_target_info = gathered_target_info_list[0] if gathered_target_info_list else {}

    # 2) Prompt list broadcast
    prompts_container = [all_prompts_for_generation] if accelerator.is_main_process else [None]
    broadcast_object_list(prompts_container)
    complete_prompt_list = prompts_container[0] if prompts_container else []

    # 3) Slice the list so that each rank gets its shard
    prompts_for_this_process = complete_prompt_list[accelerator.process_index::accelerator.num_processes]
    ...
    if accelerator.is_main_process:
        ...
        for proc_result_dict in gathered_results:
            final_generated_data.update(proc_result_dict)

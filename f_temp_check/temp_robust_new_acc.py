...
def run_generation_phase(args, accelerator): # Pass accelerator
    ...
    try:
        # Load model standardly; Accelerator handles device placement via `prepare`
        _model, _tokenizer, loaded_model_name, _ = load_model_and_tokenizer(args.model_path)
    ...
    # Revised: Each process loads the data independently.
    ...
    if accelerator.is_main_process:
        try:
            ...
            # --- 4. Prepare Prompts & Target Info (on each process) ---
            questions_processed_count = 0
            logging.info(f"Preparing prompts on process {accelerator.process_index}...")
            for item in target_questions_data:
                ...
                if qid not in original_completions_map:
                    logging.warning(f"(Process {accelerator.process_index}) QID {qid} from hint verification not found in original completions. Skipping.")
                    continue
                if qid not in hints_options_map:
                    logging.warning(f"(Process {accelerator.process_index}) Hint option for QID {qid} not found in switch analysis. Skipping.")
                    continue

                original_completion_text = original_completions_map[qid]
                prompt_text = extract_prompt_text(original_completion_text)
                if prompt_text is None:
                    logging.warning(f"(Process {accelerator.process_index}) Could not extract prompt for QID {qid}. Skipping.")
                    continue
                ...
                if args.demo_mode_limit is not None and questions_processed_count >= args.demo_mode_limit:
                    logging.info(f"(Process {accelerator.process_index}) Reached demo mode limit of {args.demo_mode_limit} questions.")
                    break

            if not prompts_for_generation:
                logging.error(f"No valid prompts prepared for generation on process {accelerator.process_index}. Exiting.")
                # This process will exit, others might continue if they succeeded. Consider using accelerator.set_trigger() for hard stop.
                return

        except Exception as e:
            logging.error(f"Failed to load/prepare data on process {accelerator.process_index}: {e}")
    ...
    if accelerator.is_main_process:
        ...
        # Save config only if it hasn't been loaded from an existing analysis file (relevant if resuming)
        # We need the config from args anyway

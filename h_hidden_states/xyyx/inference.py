...
def _generate(
    ...
    extra: Dict[str, List] = {}
    if save_hidden:
        # ---------- prompt-only resid_post capture ----------
        with torch.no_grad():
            prefix_out = model(
                encodings.input_ids,
        ...
        cut_pos = [
            int(((seq == thought_id).nonzero(as_tuple=False)[0, 0] + 1).item())
            if (seq == thought_id).any() else seq.size(0)
            for seq in encodings.input_ids
        ]

        layer_idxs = list(range(4, 33, 4))   # 4, 8, 12 … 32
        hidden = []
        for b in range(len(prompts)):
            per_layer = [
                prefix_out.hidden_states[L][b, :cut_pos[b]].to(torch.float16).cpu()
                for L in layer_idxs
            ]
